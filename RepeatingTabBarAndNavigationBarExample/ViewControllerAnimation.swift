...
class ViewControllerAnimation: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        destinationView.transform = CGAffineTransform(translationX: direction * destinationView.frame.width, y: 0.0)
        transitionContext.containerView.addSubview(destinationView)
        
        UIView.animate(withDuration: transitionDuration(using: transitionContext), animations: { destinationView.transform = .identity}, completion: { transitionContext.completeTransition($0) })
    }
}
